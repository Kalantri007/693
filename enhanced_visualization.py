...
"""
Enhanced visualization script for DFS vs BFS comparison
This script generates more visually appealing and informative visualizations
"""
...
def create_execution_time_comparison(synthetic_df, facebook_df, output_path):
    ...
    axes[0].set_title('Execution Time: Synthetic Network', fontsize=14, pad=10)
    sns.violinplot(ax=axes[0], data=[dfs_synth, bfs_synth], palette=colors)
    ...
    axes[0].axhline(y=dfs_mean, color=colors[1], linestyle='--', alpha=0.7)
    axes[0].axhline(y=bfs_mean, color=colors[0], linestyle='--', alpha=0.7)
    ...
    axes[1].set_title('Execution Time: Real Facebook Network', fontsize=14, pad=10)
    sns.violinplot(ax=axes[1], data=[dfs_fb, bfs_fb], palette=colors)
    ...
    axes[1].axhline(y=dfs_mean, color=colors[1], linestyle='--', alpha=0.7)
    axes[1].axhline(y=bfs_mean, color=colors[0], linestyle='--', alpha=0.7)
...
def create_recommendation_count_comparison(synthetic_df, facebook_df, output_path):
    ...
    # Synthetic network plot
    axes[0].set_title('Recommendation Count: Synthetic Network', fontsize=14, pad=10)
    sns.barplot(ax=axes[0], x=['DFS', 'BFS'], y=[dfs_synth.mean(), bfs_synth.mean()], palette=colors)
    axes[0].set_ylabel('Average Number of Recommendations', fontsize=12)
    
    # Add error bars
    axes[0].errorbar(x=[0, 1], y=[dfs_synth.mean(), bfs_synth.mean()], 
    ...
    for i, mean in enumerate([dfs_synth.mean(), bfs_synth.mean()]):
        axes[0].text(i, mean + 0.1, f"{mean:.1f}", ha='center', fontsize=10)
    
    # Facebook network plot
    axes[1].set_title('Recommendation Count: Real Facebook Network', fontsize=14, pad=10)
    sns.barplot(ax=axes[1], x=['DFS', 'BFS'], y=[dfs_fb.mean(), bfs_fb.mean()], palette=colors)
    axes[1].set_ylabel('Average Number of Recommendations', fontsize=12)
    
    # Add error bars
    axes[1].errorbar(x=[0, 1], y=[dfs_fb.mean(), bfs_fb.mean()], 
                 yerr=[dfs_fb.std(), bfs_fb.std()], 
                 fmt='none', capsize=5, color='black', alpha=0.7)
    
    # Annotate bars
    for i, mean in enumerate([dfs_fb.mean(), bfs_fb.mean()]):
        axes[1].text(i, mean + 30, f"{mean:.1f}", ha='center', fontsize=10)
...
def create_similarity_distribution(synthetic_df, facebook_df, output_path):
    ...
    # Create custom colormap for a gradient effect
    cmap = LinearSegmentedColormap.from_list('blue_to_red', ['#3498db', '#e74c3c'])
    
    # Synthetic network plot
    axes[0].set_title('Recommendation Similarity: Synthetic Network', fontsize=14, pad=10)
    sns.histplot(ax=axes[0], x=sim_synth, bins=10, kde=True, color='#3498db', alpha=0.7, edgecolor='black')
    axes[0].set_xlabel('Jaccard Similarity Score', fontsize=12)
    axes[0].set_ylabel('Frequency', fontsize=12)
    
    # Add mean line
    axes[0].axvline(x=sim_synth.mean(), color='red', linestyle='--', linewidth=2, alpha=0.8)
    axes[0].text(sim_synth.mean() - 0.02, axes[0].get_ylim()[1] * 0.9, f"Mean: {sim_synth.mean():.3f}", 
             rotation=90, va='top', ha='right', color='red', fontweight='bold',
             bbox=dict(boxstyle="round,pad=0.3", fc="#ffffff", ec="gray", alpha=0.8))
    
    # Facebook network plot
    axes[1].set_title('Recommendation Similarity: Real Facebook Network', fontsize=14, pad=10)
    sns.histplot(ax=axes[1], x=sim_fb, bins=10, kde=True, color='#e74c3c', alpha=0.7, edgecolor='black')
    axes[1].set_xlabel('Jaccard Similarity Score', fontsize=12)
    axes[1].set_ylabel('Frequency', fontsize=12)
    
    # Add mean line
    axes[1].axvline(x=sim_fb.mean(), color='blue', linestyle='--', linewidth=2, alpha=0.8)
    axes[1].text(sim_fb.mean() + 0.02, axes[1].get_ylim()[1] * 0.9, f"Mean: {sim_fb.mean():.3f}", 
             rotation=90, va='top', ha='left', color='blue', fontweight='bold',
             bbox=dict(boxstyle="round,pad=0.3", fc="#ffffff", ec="gray", alpha=0.8))
...
def create_combined_performance_visualization(synthetic_df, facebook_df, output_path):
    ...
    time_data = pd.DataFrame({
        'Network': ['Synthetic'] * len(dfs_synth_time) * 2 + ['Facebook'] * len(dfs_fb_time) * 2,
        'Algorithm': ['DFS'] * len(dfs_synth_time) + ['BFS'] * len(bfs_synth_time) + 
                    ['DFS'] * len(dfs_fb_time) + ['BFS'] * len(bfs_fb_time),
        'Time (s)': list(dfs_synth_time) + list(bfs_synth_time) + list(dfs_fb_time) + list(bfs_fb_time)
    })
    sns.boxplot(ax=axes[0, 0], x='Network', y='Time (s)', hue='Algorithm', 
               data=time_data, palette=colors)
    ...
    rec_data = pd.DataFrame({
        'Network': ['Synthetic'] * len(dfs_synth_recs) * 2 + ['Facebook'] * len(dfs_fb_recs) * 2,
        'Algorithm': ['DFS'] * len(dfs_synth_recs) + ['BFS'] * len(bfs_synth_recs) + 
                    ['DFS'] * len(dfs_fb_recs) + ['BFS'] * len(bfs_fb_recs),
        'Count': list(dfs_synth_recs) + list(bfs_synth_recs) + list(dfs_fb_recs) + list(bfs_fb_recs)
    })
    sns.boxplot(ax=axes[0, 1], x='Network', y='Count', hue='Algorithm', 
               data=rec_data, palette=colors)
    axes[0, 1].set_ylabel('Number of Recommendations', fontsize=12)
    axes[0, 1].legend(title='Algorithm')
    
    # Bottom left: Similarity distribution
    axes[1, 0].set_title('Recommendation Similarity Distribution', fontsize=14)
    sim_data = pd.DataFrame({
        'Network': ['Synthetic'] * len(sim_synth) + ['Facebook'] * len(sim_fb),
        'Similarity': list(sim_synth) + list(sim_fb)
    })
    sns.violinplot(ax=axes[1, 0], x='Network', y='Similarity', data=sim_data, 
